...
# Description:
#   This script renders and merges Talos machine configurations using minijinja-cli, vault and talosctl.
...
function main() {
    ...
    if ! base=$(minijinja-cli --define "machinetype=${type}" "${MACHINEBASE}" | task --silent vault:inject) || [[ -z "${base}" ]]; then
    ...
    if ! patch=$(minijinja-cli --define "machinetype=${type}" "${MACHINEPATCH}" | task --silent vault:inject) || [[ -z "${patch}" ]]; then
