...
function apply_secrets() {
    ...
    for secret in "${cluster_secrets[@]}"; do
        if [[ -f "${ROOT_DIR}/bootstrap/${cluster}/${secret}" ]]; then
            # Check if the secret resources are up-to-date
            if sops exec-file "${ROOT_DIR}/bootstrap/${cluster}/${secret}" "kubectl --context ${cluster} --namespace flux-system diff --filename {}" &>/dev/null; then
                log info "Secret resource is up-to-date" "resource=$(basename "${ROOT_DIR}/bootstrap/${cluster}/${secret}" ".sops.yaml")"
                continue
            fi
            if sops --decrypt ${ROOT_DIR}/bootstrap/${cluster}/${secret} | kubectl apply --context ${cluster} --server-side --filename - &>/dev/null; then
                log info "secret applied" "secret=${secret}"
            else
                log error "Failed to apply secret" "secret=${secret}"
            fi
        else
            log info "File does not exist" "file=${ROOT_DIR}/bootstrap/${cluster}/${secret}"
        fi
    done
...
function apply_configmaps() {
    ...
    local -r configmaps=(
        "${ROOT_DIR}/kubernetes/vars/${cluster}/cluster-settings.yaml"
    )
